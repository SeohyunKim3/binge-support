                ...
                {grouped[dayKey].map((it, idx) => (
                  <li key={it.id} style={{ padding: '12px 0', borderBottom: '1px solid #eee' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <button onClick={() => router.push(`/dashboard/entry/${it.id}`)}>Edit</button>
                      <button onClick={() => removeEntry(it.id)}>Delete</button>
                      <button onClick={() => togglePublic(it.id, !(it.is_public ?? false))}>
